...
//modified from UnleashedRecomp
// I couldnt be bothered to remove the other languages from unleahedrecomp nor could i be bothered adding them into config_locale
// so ig there will be english only but the other languages are written here
std::unordered_map<std::string_view, std::unordered_map<ELanguage, std::string>> g_locale =
{
    ...
    },
    {
        ...
        {
            { ELanguage::English,  "The module \"%s\" could not be found.\n\nPlease make sure that:\n\n- You extracted this copy of GTA4Recomp fully and not just the *.exe file.\n- You are not running GTA4Recomp from a *.zip file." },
            { ELanguage::Japanese, "モジュール\"%s\"が見つかりませんでした\n\n次の点を確認してください：\n\n※GTA4Recompの*.exeファイルだけを抽出していなく、 コピーを完全に抽出してること\n※GTA4Recompを*.zipファイルから実行していないこと" },
            { ELanguage::German,   "Das Modul \"%s\" konnte nicht gefunden werden.\n\nBitte stelle sicher, dass:\n\n- Diese Kopie von GTA4Recomp vollständig entpackt wurde und nicht nur die *.exe-Datei.\n- GTA4Recomp nicht direkt aus einer *.zip-Datei ausgeführt wird." },
            { ELanguage::French,   "Le module \"%s\" n'a pas pu être trouvé.\n\nVeuillez vous assurer que :\n\n- Vous avez extrait GTA4Recomp dans son entièreté et pas seulement le fichier *.exe.\n- Vous n'exécutez pas GTA4Recomp à partir d'un fichier *.zip." },
            { ELanguage::Spanish,  "No se pudo encontrar el módulo \"%s\".\n\nAsegúrese de que:\n\n- Ha extraido esta copia de GTA4Recomp por completo y no solo el archivo *.exe.\n- No está ejecutando GTA4Recomp desde un archivo *.zip." },
            { ELanguage::Italian,  "Impossibile trovare il modulo \"%s\".\n\nAssicurati che:\n\n- Hai estratto questa copia di GTA4Recomp correttamente e non solo il file *.exe.\n- Non stai eseguendo GTA4Recomp da un file *.zip." }
        }
    },
    {
