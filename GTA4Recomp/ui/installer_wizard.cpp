...
#include <res/images/common/GTA4Recomp.dds.h>
...
constexpr float CONTAINER_X = 620.0f;  
constexpr float CONTAINER_Y = 180.0f;  
constexpr float CONTAINER_WIDTH = 480.0f;
constexpr float CONTAINER_HEIGHT = 380.0f;
...
static std::array<ImVec2, 8> g_installTexturePositions = { ImVec2(-100.0f, 120.0f),
...
static std::string& GetWizardText(WizardPage page)
{
    switch (page)
    {
        ...
        default: return Localise("Installer_Page_SelectGame");
...
static void DrawBackground()
{
    ...
    drawList->AddRectFilled({ 0.0, 0.0 }, res, Style::Colors::Background);
}

static void DrawGTA4Logo()
{
    // Simplified: no logo for plain installer
    (void)g_upGTA4Logo;
}
...
static void DrawGTA4Header()
{
    ...
    drawList->AddRectFilled({ 0, stripY }, { res.x, stripY + stripHeight }, Style::WithAlpha(Style::Colors::Red, alpha));
    ...
    float titleX = (res.x / 2.0f) - (titleSize.x / 2.0f);
    float titleY = stripY + (stripHeight - titleSize.y) / 2.0f;

    drawList->AddText(g_pFntNewRodin, titleFontSize, { titleX, titleY }, IM_COL32(255, 255, 255, (int)(255 * alpha)), title);
}
...
{
    (void)g_installTextures;
    (void)g_installTexturePositions;
    (void)IMAGE_WIDTH;
    (void)IMAGE_HEIGHT;
}
...
static void DrawButtonContainer(ImVec2 min, ImVec2 max, int baser, int baseg, float alpha)
{
    ...
    ImU32 bgColor = isHovered 
        ? Style::WithAlpha(IM_COL32(50, 50, 50, 255), alpha)
        : Style::WithAlpha(IM_COL32(30, 30, 30, 200), alpha);
...
static void DrawButton(ImVec2 min, ImVec2 max, const char *buttonText, bool sourceButton, bool buttonEnabled, bool &buttonPressed, float maxTextWidth = FLT_MAX, bool makeDefault = false)
{
    ...
    ImU32 textColor = isHovered 
        ? Style::WithAlpha(Style::Colors::Red, alpha)
...
static void DrawProgressBar(float progressRatio)
{
    ...
    const uint32_t innerColor0 = Style::Colors::ProgressBarBG;
    const uint32_t innerColor1 = Style::Colors::BackgroundDark;
    ...
    drawList->AddRectFilled(min, max, Style::Colors::BackgroundPanel);
    drawList->AddRect(min, max, Style::Colors::Border);
    ...
    const uint32_t sliderColor0 = Style::Colors::ProgressBarFill;
    const uint32_t sliderColor1 = Style::Colors::ProgressBarFillAlt;
    xPadding += Scale(1.5f);
    yPadding += Scale(1.5f);
...
static void DrawGTA4DLCSelection()
{
    ...
    const char* msg = "DLC selection is not supported in this build.";
    float fontSize = Scale(20);
    ImVec2 txtSize = g_pFntNewRodin->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, msg);
    ImVec2 pos = { (res.x - txtSize.x) / 2.0f, (res.y - txtSize.y) / 2.0f };
    drawList->AddText(g_pFntNewRodin, fontSize, pos, IM_COL32(200,200,200,(int)(200*alpha)), msg);

    float hintY = res.y - Scale(40);
    ImFont* hintFont = g_pFntRodin;
    float hintFontSize = Scale(14);
    drawList->AddText(hintFont, hintFontSize, { Scale(40), hintY }, Style::WithAlpha(Style::Colors::GreyText, alpha), "[ESC] BACK");
    drawList->AddText(hintFont, hintFontSize, { res.x - Scale(200), hintY }, Style::WithAlpha(Style::Colors::GreyText, alpha), "[CLICK] SELECT");
}
...
bool InstallerWizard::Run(std::filesystem::path installPath, bool skipGame)
{
    ...
    if (skipGame)
    {
        g_firstPage = WizardPage::SelectGame;
    }
