...
void CommonMenu::Draw()
{
    ...
    auto redStripColour = Style::Colors::Red;
    ...
    auto gradientTop = Style::Colors::BackgroundDark;
    auto gradientBottom = Style::Colors::Background;
    auto gradientHeight = Scale(120, true);

    if (!ReduceDraw)
    {
        drawList->AddRectFilled({ 0.0f, 0.0f }, { res.x, topPlateMotion + gradientHeight }, Style::Colors::Background);
        drawList->AddRectFilled({ 0.0f, redStripCornerMin.y }, { res.x, redStripCornerMax.y }, redStripColour);

    }

    auto titleText = Title.empty() ? "GTA4Recomp" : Title.data();
    ...
    auto textCoverColour = Style::Colors::BackgroundPanel;
    ...
    else
    {
        ...
        drawList->AddRectFilled({ 0.0f, bottomPlateMotion }, { res.x, res.y }, Style::Colors::Background);
    }
