...
#if !defined(FERNANDO_RECOMP_D3D12) && !defined(FERNANDO_RECOMP_METAL)
...
static GuestShader* CreateShader(const be<uint32_t>* function, ResourceType resourceType)
{
    ...
    {
        LOG_ERROR("Shader cache is empty (g_shaderCacheEntryCount == 0).");
        LOG_ERROR("no GTA4RecompLib/shader/shader_cache.cpp? you should prolly add it");
        std::_Exit(1);
    }
...
// Find fxc files and create all shaders
        ...
        if (bytecodePtr != nullptr) {
            ...
            if ((flags & 0xFFFFFF00) == 0x102A1100 && virtualSize > 0 && physicalSize > 0) {
                ...
                if (entry != nullptr) {
                    ...
                    // return the cached shader handle
...
// from original marathon sonic06, its not used in gta4 but I need to keep if for references
