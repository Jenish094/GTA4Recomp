...
#define FERNANDO_CONCAT2(x, y) x##y
#define FERNANDO_CONCAT(x, y) FERNANDO_CONCAT2(x, y)

#define FERNANDO_INSERT_PADDING(length) \
    uint8_t FERNANDO_CONCAT(pad, __LINE__)[length]
#define FERNANDO_ASSERT_OFFSETOF(type, field, offset) \
    static_assert(offsetof(type, field) == offset)

#define FERNANDO_ASSERT_SIZEOF(type, size) \
    static_assert(sizeof(type) == size)

#define FERNANDO_VIRTUAL_FUNCTION(returnType, virtualIndex, ...) \
    GuestToHostFunction<returnType>(*(be<uint32_t>*)(g_memory.Translate(*(be<uint32_t>*)(this) + (4 * virtualIndex))), __VA_ARGS__)

struct fernando_null_ctor {};
